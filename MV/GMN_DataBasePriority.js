 ...
 * Example description:
 * <priority:100>.
 * The priority tag name ("priority") can be changed in the plugin param.
 ...
 * Ascending order (1,2,3...) and descending order (99999,99998,99997...) can be selected by plugin param.
...
"use strict";
{
  ...
  const defaultPriority = Number(param.defaultPriority);
  const priorityName = param.priorityName;
  const getPriority = (data) => {
    const priority = Number(data.meta[priorityName]);
    return Number.isInteger(priority) ? priority : defaultPriority;
  };
